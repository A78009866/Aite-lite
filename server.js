...
app.get('/group_chat', requireAuth, (req, res) => { res.sendFile(path.join(__dirname, 'views', 'group_chat.html')); });
...
// --- Create Group Chat ---
app.post('/api/groups/create', writeLimiter, upload.single('group_image'), requireAuth, async (req, res) => {
  try {
    const adminId = req.session.userId;
    const { name, member_ids } = req.body;
    if (!name || !name.trim()) return res.status(400).json({ ok: false, error: 'اسم المجموعة مطلوب' });

    let members = [];
    if (typeof member_ids === 'string') {
      try { members = JSON.parse(member_ids); } catch(e) { members = member_ids.split(',').map(s => s.trim()).filter(Boolean); }
    } else if (Array.isArray(member_ids)) {
      members = member_ids;
    }
    if (members.length === 0) return res.status(400).json({ ok: false, error: 'يجب إضافة عضو واحد على الأقل' });

    // Verify all members are friends
    for (const mid of members) {
      const isFriend = await areFriends(adminId, mid);
      if (!isFriend) return res.status(400).json({ ok: false, error: `العضو ${mid} ليس من أصدقائك` });
    }

    let imageUrl = 'https://res.cloudinary.com/duixjs8az/image/upload/v1765009560/post_media/1765009560909-default_profile.png';
    ...
    const allMembers = [adminId, ...members];
    const membersObj = {};
    allMembers.forEach(id => { membersObj[id] = { joined_at: Date.now(), role: id === adminId ? 'admin' : 'member' }; });
    ...
    allMembers.forEach(mid => {
      updates[`userGroups/${mid}/${groupId}`] = { joined_at: Date.now() };
    });
    await db.ref().update(updates);

    // Send notification to members
    const adminProfile = (await db.ref(`profiles/${adminId}`).once('value')).val() || {};
    for (const mid of members) {
      try {
        const notifRef = db.ref(`notifications/${mid}`).push();
        await notifRef.set({
          id: notifRef.key,
          type: 'group_added',
          from_user_id: adminId,
          from_username: adminProfile.username || 'مستخدم',
          from_profile_picture_url: adminProfile.profile_picture_url || DEFAULT_PROFILE_PIC_URL,
          group_name: name.trim(),
          group_id: groupId,
          timestamp: admin.database.ServerValue.TIMESTAMP,
          is_read: false
        });
      } catch(e) { console.error('Group notif error:', e); }
    }

    res.json({ ok: true, group: groupData });
  } catch (error) {
    console.error('Error creating group:', error);
    res.status(500).json({ ok: false, error: 'فشل في إنشاء المجموعة' });
  }
});
...
app.get('/api/groups', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const userGroupsSnap = await db.ref(`userGroups/${userId}`).once('value');
    const userGroups = userGroupsSnap.val() || {};
    const groupIds = Object.keys(userGroups);
    if (groupIds.length === 0) return res.json({ ok: true, groups: [] });

    const groups = [];
    for (const gid of groupIds) {
      const gSnap = await db.ref(`groupChats/${gid}`).once('value');
      const g = gSnap.val();
      if (!g) continue;
      // Check if user is still a member
      if (!g.members || !g.members[userId]) continue;
      // Get last message
      const lastMsgSnap = await db.ref(`groupMessages/${gid}`).orderByChild('timestamp').limitToLast(1).once('value');
      let lastMessage = null;
      lastMsgSnap.forEach(child => { lastMessage = child.val(); });
      // Get unread count
      const unreadSnap = await db.ref(`groupUnread/${gid}/${userId}`).once('value');
      ...
      groups.push({
        ...
        member_count: Object.keys(g.members || {}).length
      });
    }
    res.json({ ok: true, groups });
  } catch (error) {
    console.error('Error fetching groups:', error);
    res.status(500).json({ ok: false, error: 'فشل في جلب المجموعات' });
  }
});
...
app.get('/api/groups/:groupId', requireAuth, async (req, res) => {
  try {
    ...
    const gSnap = await db.ref(`groupChats/${groupId}`).once('value');
    const g = gSnap.val();
    if (!g) return res.status(404).json({ ok: false, error: 'المجموعة غير موجودة' });
    if (!g.members || !g.members[userId]) return res.status(403).json({ ok: false, error: 'لست عضوا في هذه المجموعة' });

    // Enrich members with profile data
    const memberIds = Object.keys(g.members);
    const memberProfiles = {};
    const profileSnaps = await Promise.all(memberIds.map(id => db.ref(`profiles/${id}`).once('value')));
    profileSnaps.forEach((snap, idx) => {
      const p = snap.val() || {};
      memberProfiles[memberIds[idx]] = {
        id: memberIds[idx],
        username: p.username || 'مستخدم',
        full_name: p.full_name || '',
        profile_picture_url: p.profile_picture_url || DEFAULT_PROFILE_PIC_URL,
        ...
        role: g.members[memberIds[idx]].role || 'member'
      };
    });

    res.json({ ok: true, group: { ...g, memberProfiles } });
  } catch (error) {
    console.error('Error fetching group:', error);
    res.status(500).json({ ok: false, error: 'فشل في جلب بيانات المجموعة' });
  }
});
...
app.get('/api/groups/:groupId/messages', requireAuth, async (req, res) => {
  try {
    ...
    const groupId = sanitizePathParam(req.params.groupId);
    // Verify membership
    const memberSnap = await db.ref(`groupChats/${groupId}/members/${userId}`).once('value');
    if (!memberSnap.exists()) return res.status(403).json({ ok: false, error: 'لست عضوا في هذه المجموعة' });

    const msgsSnap = await db.ref(`groupMessages/${groupId}`).orderByChild('timestamp').once('value');
    const messages = [];
    msgsSnap.forEach(child => { messages.push(child.val()); });

    // Reset unread count
    await db.ref(`groupUnread/${groupId}/${userId}`).set(0);
...
// --- Send group message ---
app.post('/api/groups/:groupId/messages/send', writeLimiter, (req, res, next) => {
  const ct = req.headers['content-type'] || '';
  ...
  try {
    ...
    // Verify membership
    const memberSnap = await db.ref(`groupChats/${groupId}/members/${senderId}`).once('value');
    if (!memberSnap.exists()) return res.status(403).json({ ok: false, error: 'لست عضوا في هذه المجموعة' });
    ...
    let mediaObject = null;
    if (req.body.mediaUrl && isValidMediaUrl(req.body.mediaUrl)) {
      mediaObject = { url: req.body.mediaUrl, type: req.body.mediaType || 'file', filename: req.body.mediaFilename || 'media' };
    }
    if (!mediaObject && files.length > 0) {
      const file = files[0];
      let type = 'file';
      ...
      mediaObject = { url: file.path, type, filename: file.originalname };
    }

    if (!content.trim() && !mediaObject) {
      return res.status(400).json({ ok: false, error: 'لا يمكن إرسال رسالة فارغة' });
    }
    ...
    // Get sender profile
    const senderProfile = (await db.ref(`profiles/${senderId}`).once('value')).val() || {};

    const newMessage = {
      id: messageId,
      messageId,
      senderId,
      senderName: senderProfile.username || 'مستخدم',
      senderPic: senderProfile.profile_picture_url || DEFAULT_PROFILE_PIC_URL,
      content,
      media: mediaObject,
      timestamp: admin.database.ServerValue.TIMESTAMP,
      replied_to_id: reply_to_id,
    ...
    const groupSnap = await db.ref(`groupChats/${groupId}/members`).once('value');
    const members = groupSnap.val() || {};
    const updates = {};
    for (const mid of Object.keys(members)) {
      if (mid !== senderId) {
        updates[`groupUnread/${groupId}/${mid}`] = admin.database.ServerValue.increment(1);
      }
    }
    if (Object.keys(updates).length > 0) await db.ref().update(updates);

    // Update group's last_message for list display
    let previewText = content;
    if (!content && mediaObject) {
      previewText = mediaObject.type === 'image' ? 'صورة' : mediaObject.type === 'video' ? 'فيديو' : mediaObject.type === 'audio' ? 'رسالة صوتية' : 'ملف مرفق';
    }
    await db.ref(`groupChats/${groupId}/last_message`).set({
      content: previewText,
      timestamp: Date.now(),
      senderId,
      senderName: senderProfile.username || 'مستخدم'
...
app.post('/api/groups/:groupId/mark_read', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    await db.ref(`groupUnread/${groupId}/${userId}`).set(0);
...
app.post('/api/groups/:groupId/members/add', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    const { member_id } = req.body;
    if (!member_id) return res.status(400).json({ ok: false, error: 'member_id مطلوب' });

    const gSnap = await db.ref(`groupChats/${groupId}`).once('value');
    const g = gSnap.val();
    if (!g) return res.status(404).json({ ok: false, error: 'المجموعة غير موجودة' });
    if (g.admin_id !== userId) return res.status(403).json({ ok: false, error: 'فقط المسؤول يمكنه إضافة أعضاء' });

    // Check friendship
    const isFriend = await areFriends(userId, member_id);
    if (!isFriend) return res.status(400).json({ ok: false, error: 'يجب أن يكون العضو من أصدقائك' });

    // Already a member?
    if (g.members && g.members[member_id]) return res.status(400).json({ ok: false, error: 'العضو موجود بالفعل في المجموعة' });

    await db.ref(`groupChats/${groupId}/members/${member_id}`).set({ joined_at: Date.now(), role: 'member' });
    await db.ref(`userGroups/${member_id}/${groupId}`).set({ joined_at: Date.now() });

    res.json({ ok: true });
  } catch (error) {
    console.error('Error adding member:', error);
    res.status(500).json({ ok: false, error: 'فشل في إضافة العضو' });
  }
});

// --- Admin: Remove member from group ---
app.post('/api/groups/:groupId/members/remove', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    const { member_id } = req.body;
    ...
    const g = gSnap.val();
    if (!g) return res.status(404).json({ ok: false, error: 'المجموعة غير موجودة' });
    if (g.admin_id !== userId) return res.status(403).json({ ok: false, error: 'فقط المسؤول يمكنه إزالة أعضاء' });
    if (member_id === userId) return res.status(400).json({ ok: false, error: 'لا يمكنك إزالة نفسك' });

    await db.ref(`groupChats/${groupId}/members/${member_id}`).remove();
    await db.ref(`userGroups/${member_id}/${groupId}`).remove();
    await db.ref(`groupUnread/${groupId}/${member_id}`).remove();

    res.json({ ok: true });
  } catch (error) {
    console.error('Error removing member:', error);
    res.status(500).json({ ok: false, error: 'فشل في إزالة العضو' });
  }
});
...
app.post('/api/groups/:groupId/leave', requireAuth, async (req, res) => {
  try {
    ...
    const g = gSnap.val();
    if (!g) return res.status(404).json({ ok: false, error: 'المجموعة غير موجودة' });
    if (g.admin_id === userId) return res.status(400).json({ ok: false, error: 'لا يمكن للمسؤول مغادرة المجموعة' });

    await db.ref(`groupChats/${groupId}/members/${userId}`).remove();
    await db.ref(`userGroups/${userId}/${groupId}`).remove();
    await db.ref(`groupUnread/${groupId}/${userId}`).remove();

    res.json({ ok: true });
  } catch (error) {
    console.error('Error leaving group:', error);
    res.status(500).json({ ok: false, error: 'فشل في مغادرة المجموعة' });
  }
});

// --- Admin: Update group info (name/image) ---
app.post('/api/groups/:groupId/update', upload.single('group_image'), requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    ...
    const g = gSnap.val();
    if (!g) return res.status(404).json({ ok: false, error: 'المجموعة غير موجودة' });
    if (g.admin_id !== userId) return res.status(403).json({ ok: false, error: 'فقط المسؤول يمكنه تعديل المجموعة' });

    const updates = {};
    if (req.body.name && req.body.name.trim()) {
      updates[`groupChats/${groupId}/name`] = req.body.name.trim();
    }
    if (req.file && req.file.path) {
      updates[`groupChats/${groupId}/image`] = req.file.path;
    }
    ...
    const updated = (await db.ref(`groupChats/${groupId}`).once('value')).val();
    res.json({ ok: true, group: updated });
  } catch (error) {
    console.error('Error updating group:', error);
    res.status(500).json({ ok: false, error: 'فشل في تحديث المجموعة' });
  }
});

// --- Delete message in group ---
app.post('/api/groups/:groupId/messages/delete', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    const { message_id } = req.body;
    if (!message_id) return res.status(400).json({ ok: false, error: 'message_id مطلوب' });
    ...
    if (msg.senderId !== userId) return res.status(403).json({ ok: false, error: 'لا يمكنك حذف رسالة شخص آخر' });

    await db.ref(`groupMessages/${groupId}/${message_id}`).update({ is_deleted: true, content: '', media: null });
    res.json({ ok: true });
  } catch (error) {
...
app.post('/api/groups/:groupId/messages/react', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    ...
    const memberSnap = await db.ref(`groupChats/${groupId}/members/${userId}`).once('value');
    if (!memberSnap.exists()) return res.status(403).json({ ok: false });

    if (reaction) {
      await db.ref(`groupMessages/${groupId}/${message_id}/reaction`).set(reaction);
    } else {
      await db.ref(`groupMessages/${groupId}/${message_id}/reaction`).remove();
    }
    res.json({ ok: true });
  } catch (error) {
...
app.post('/api/groups/:groupId/typing', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    const { typing } = req.body;
    if (typing) {
      await db.ref(`groupTyping/${groupId}/${userId}`).set(Date.now());
    } else {
      await db.ref(`groupTyping/${groupId}/${userId}`).remove();
    }
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ ok: false }); }
});

app.get('/api/groups/:groupId/typing', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    const groupId = sanitizePathParam(req.params.groupId);
    const snap = await db.ref(`groupTyping/${groupId}`).once('value');
    const typing = snap.val() || {};
    const now = Date.now();
    const active = {};
    for (const [uid, ts] of Object.entries(typing)) {
      if (uid !== userId && (now - ts) < 5000) active[uid] = true;
    }
