...
(function() {
  ...
  // i18n helper
  var t = (window.AiteI18n && window.AiteI18n.t) ? window.AiteI18n.t : function(s){return s;};
    ...
    overlay.innerHTML = '<style>' +
      ...
      '<div style="background:rgba(20,20,20,0.95);border:1px solid rgba(255,255,255,0.1);border-radius:20px;padding:28px;max-width:400px;width:100%;animation:recModalSlideUp 0.3s ease;box-shadow:0 20px 60px rgba(0,0,0,0.6);">' +
        '<div style="text-align:center;margin-bottom:20px;">' +
          ...
          '</div>' +
          '<h3 data-i18n="أضف بريد الاستعادة" style="color:#fff;font-size:18px;font-weight:700;margin:0 0 6px;">' + t('أضف بريد الاستعادة') + '</h3>' +
          '<p data-i18n="أضف بريدك الإلكتروني لاستعادة حسابك في حال نسيت كلمة المرور" style="color:#9ca3af;font-size:13px;margin:0;">' + t('أضف بريدك الإلكتروني لاستعادة حسابك في حال نسيت كلمة المرور') + '</p>' +
        '</div>' +
        ...
        '<button id="recEmailSaveBtn" style="width:100%;padding:12px;border-radius:12px;background:linear-gradient(135deg,#3982f7,#2b61b8);color:#fff;font-weight:700;font-size:15px;border:none;cursor:pointer;margin-bottom:8px;transition:all 0.3s;" data-i18n="حفظ البريد">' + t('حفظ البريد') + '</button>' +
        '<button id="recEmailDismissBtn" style="width:100%;padding:10px;border-radius:12px;background:transparent;color:#6b7280;font-size:13px;border:1px solid rgba(255,255,255,0.08);cursor:pointer;transition:all 0.3s;" data-i18n="لاحقاً">' + t('لاحقاً') + '</button>' +
      '</div>';
  ...
  function saveEmail() {
    ...
    if (!email) { errorEl.textContent = t('أدخل البريد الإلكتروني.'); errorEl.style.display = 'block'; return; }
    var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) { errorEl.textContent = t('صيغة البريد غير صحيحة.'); errorEl.style.display = 'block'; return; }

    saveBtn.disabled = true;
    saveBtn.textContent = t('جارٍ الحفظ...');
    ...
    .then(function(data) {
      if (data.ok) {
        successEl.textContent = data.message || t('تم الحفظ بنجاح!');
        successEl.style.display = 'block';
        setTimeout(dismissModal, 1500);
      } else {
        errorEl.textContent = data.error || t('حدث خطأ.');
        errorEl.style.display = 'block';
        saveBtn.disabled = false;
        saveBtn.textContent = t('حفظ البريد');
      }
    })
    .catch(function() {
      errorEl.textContent = t('فشل في الاتصال.');
      errorEl.style.display = 'block';
      saveBtn.disabled = false;
      saveBtn.textContent = t('حفظ البريد');
    });
